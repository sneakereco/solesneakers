...
2. Log, then rate-limit `POST /api/checkout/payment-link` by IP at 3 requests per 10 minutes.
3. Log, then rate-limit the same exact method/path by JA4 Digest at 5 requests per 10 minutes.
4. Challenge payment-link creation when the request country is not the United States. Do not country-block catalog browsing.
...
| Checkout IP burst limit                    | Exact path and POST; IP key; 3/10m  | Rate limit                   | TBD   | TBD              | TBD          | TBD   |
| Checkout JA4 burst limit                   | Exact path and POST; JA4 key; 5/10m | Rate limit                   | TBD   | TBD              | TBD          | TBD   |
| Non-US payment-link creation               | Exact path and POST; country not US | Challenge                    | TBD   | TBD              | TBD          | TBD   |
...
- Repeating an idempotency key for the same unexpired checkout returns the existing Square URL without consuming a new quota.
- The expiration job deletes the Square Payment Link and records that deletion before stock is released.

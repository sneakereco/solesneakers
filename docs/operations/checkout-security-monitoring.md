...
| Critical | More than 20 checkout-prepare attempts share one IP or JA4 in 10 minutes | Enable kill switch if Square objects are being created; preserve evidence                 |
...
| Critical | Payment attempts grow 3x without a corresponding paid-order increase     | Enable kill switch and inspect card-testing signals                                       |
...
| Critical | Any expiration job failure                                               | Confirm the Square link/direct order is canceled before manually releasing stock          |
...
- Prepare-to-paid conversion, permit issuance, Square declines, Risk Manager decisions, 3DS results, AVS/CVV failures, disputes, refunds, and confirmed card testing.
- Upstash usage, Vercel firewall usage, Square API errors, cron failures, and spend alerts.
- Pending reservations older than 20 minutes, Square orders canceled without released stock, and released stock whose order later received payment.
...
6. Confirm the expiration cron still cancels hosted legacy links or direct Square orders before releasing inventory.

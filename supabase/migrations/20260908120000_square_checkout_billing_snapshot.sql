...
alter function public.reserve_square_checkout_inventory(
  ...
  jsonb,
  jsonb
) rename to reserve_square_checkout_inventory_without_billing;

revoke all on function public.reserve_square_checkout_inventory_without_billing(
  uuid,
  uuid,
  text,
  text,
...
begin
  ...
  if p_payment_method in ('card', 'afterpay') and p_billing_address is null then

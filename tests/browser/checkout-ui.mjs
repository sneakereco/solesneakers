...
if (liveSquare) {
  ...
  assert.equal(
    ...
    "sandbox",
    "Live UI checks require Sandbox",
  );
  squareConfig = {
...
try {
  ...
  if (liveSquare) {
    ...
    await page.evaluate((config) => {
      ...
      window.realSquarePayments = window.Square.payments(
  ...
  if (liveSquare) {
    ...
    await page
      .getByText("Enter a valid card number", { exact: true })
      .waitFor({ state: "hidden" });
    await mkdir("test-results/checkout-ui", { recursive: true });
    await page.locator('[aria-labelledby="payment-heading"]').screenshot({
      path: "test-results/checkout-ui/square-error.png",
      animations: "disabled",
    });
    ...
    await page.getByRole("radio", { name: "Afterpay", exact: true }).click();
    await page.locator('[aria-labelledby="payment-heading"]').screenshot({
      path: "test-results/checkout-ui/afterpay.png",
      animations: "disabled",
    });
    await page.getByRole("radio", { name: "Credit card", exact: true }).click();
    ...
    await page.screenshot({
      path: "test-results/checkout-ui/square-live-desktop.png",
      fullPage: true,
      animations: "disabled",
    });
    ...
    await page.screenshot({
      path: "test-results/checkout-ui/square-live-mobile.png",
      fullPage: true,
      animations: "disabled",
    });
    await page.locator('[aria-labelledby="payment-heading"]').screenshot({
      path: "test-results/checkout-ui/square-payment-mobile.png",
      animations: "disabled",
    });
    assert.deepEqual(errors, []);
    console.log(
      "PASS: real Sandbox iframe styles, blur errors, card detection and input preservation",
    );
  } else {
    ...
    await cashPanel
      .getByText("Continue with Cash App Pay to approve your payment.")
    ...
    await mkdir("test-results/checkout-ui", { recursive: true });
    await page.screenshot({
      path: "test-results/checkout-ui/desktop.png",
      fullPage: true,
      animations: "disabled",
    });
    await page.setViewportSize({ width: 390, height: 844 });
    await page.screenshot({
      path: "test-results/checkout-ui/mobile.png",
      fullPage: true,
      animations: "disabled",
    });
    assert.equal(
    ...
    for (const [label, value] of Object.entries({
      ...
      "Billing address": "2 Market St",
      "Billing city": "Wilmington",
      "Billing ZIP code": "19801",
    })) {
    ...
    await page.evaluate(() => {
      ...
      for (const field of ["cardNumber", "expirationDate"])
        listeners.focusClassRemoved({
          detail: { field, currentState: { isEmpty: false, isCompletelyValid: true } },
        });
      listeners.focusClassAdded({
        detail: {
          field: "cvv",
          currentState: { isEmpty: true, isCompletelyValid: false },
        },
    ...
    await page.evaluate(() => {
      ...
      window.checkoutTest.listeners.focusClassAdded({
        detail: {
          field: "cvv",
          currentState: { isEmpty: true, isCompletelyValid: false },
        },
